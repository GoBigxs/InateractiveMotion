...
public class LineController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        switch(myAnimationType)
        {
            ...
            case AnimationType.Shift:
                StartCoroutine(AnimateLoop(myLineRenderer, .001f));
                break;
        }
    }
    ...
    Gradient AddInitialCopy2(Gradient incomingGradient)
    {
        List<GradientColorKey> newColorKeys = new List<GradientColorKey>(incomingGradient.colorKeys);
        
        // Ensure the newColorKeys list doesn't exceed the maximum allowed color keys by removing the last one if necessary
        if (newColorKeys.Count >= 8)
        {
            // Remove the last color key to make space for the new initial key
            newColorKeys.RemoveAt(newColorKeys.Count - 1);
        }
        
        // Use the color of the first key as the color for the new initial key
        Color interSectionColor = newColorKeys[0].color;
        newColorKeys.Insert(0, new GradientColorKey(interSectionColor, 0));
        ...
        // Optionally, handle the alphaKeys similarly if needed
        // For simplicity, this example only addresses colorKeys
    ...
    {
        lineRendererToChange.colorGradient = AddInitialCopy2(lineRendererToChange.colorGradient);
